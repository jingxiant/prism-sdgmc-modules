process ANNOTATE_VEP_SLIVAR{
        ...
        """
        echo $slivarvcf
        vep --assembly GRCh38 --refseq --exclude_predicted --cache --dir ${vep_cache_files} --dir_plugins ${params.vep_cache_plugin_dir} --fork 20 -i ${slivarvcf} -o ${slivarvcf.
baseName}.vep.vcf \
        ${params.vep_commands} \
        --plugin dbNSFP,${params.dbNSFP},PrimateAI_pred,PrimateAI_rankscore,PrimateAI_score,REVEL_rankscore,REVEL_score,Aloft_Fraction_transcripts_affected,Aloft_prob_Tolerant,Al
oft_prob_Recessive,Aloft_prob_Dominant,Aloft_pred,Aloft_Confidence \
        --plugin AlphaMissense,file=${params.AlphaMissense} \
        --plugin SpliceAI_WK2,snv=${params.SpliceAI_snp},indel=${params.SpliceAI_snp} \
        -custom ${params.gnomad_exomes},gnomADe,vcf,exact,0,AC,AF,AN,controls_AF,controls_AF_nfe,controls_AF_eas,controls_AF_asj,controls_AF_amr,controls_AF_afr,controls_AF_oth,c
ontrols_AF_sas,controls_nhomalt \
